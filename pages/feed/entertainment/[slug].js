...
const Entertainment = ({ pageNumber, articles }) => {
			...
			<div className={styles.paginator}>
				<div
					...
					onClick={() => {
						if (pageNumber > 1) {
							router
								.push(`/feed/entertainment/${pageNumber - 1}`)
...
export const getServerSideProps = async (pageContext) => {
	...
	const apiResponse = await fetch(
		`https://newsapi.org/v2/top-headlines?country=kr&category=entertainment&pageSize=5&page=${pageNumber}&apiKey=${process.env.NEWS_API_KEY}`
	);
